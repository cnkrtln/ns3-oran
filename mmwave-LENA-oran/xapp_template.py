...
def process_metrics(build_dir, sim_time):
    ...
    for cid in [1, 2]:
        ...
        for line in read_new_lines(du_file, 'du', cid):
            ...
            try:
                ts = sim_time # Use aligned simulation time
                counts = {
    ...
    # 4. Position Metrics (Distance)
    pos_file = os.path.join(build_dir, "UEPosition.txt")
    for line in read_new_lines(pos_file, 'pos'):
        p = line.strip().split(',')
        if len(p) < 7 or p[1] != 'UE': continue
        try:
            t, cid, ux, uy = float(p[0]), int(p[6]), float(p[3]), float(p[4])
            bx, by = BS_POS[cid]
            ...
            if sim_time is not None and sim_time > last_sim_time:
                ...
                if sim_time >= last_print_time + 1.0:
                    ...
                    for cid in [1, 2]:
                        ...
                        # Calculate Ping-Pong Rate for the print statement
                        tot_ho, pp_ho = 0, 0
                        for ue_id, hist in handover_history.items():
                            for ho in hist:
                                if sim_time - LOOKBACK_WINDOW < ho['time'] <= sim_time and ho['source'] == cid:
                        ...
                        print(f" Cell {cid} | SINR: {sinr:>5.1f} dB | TP: {tp:>5.0f} kbps | Lat: {lat:>4.1f} ms | Load: {load:>3.0f} UEs | Dist: {dist:>5.1f} m | PP Rate: {pp_rate:>4.1f}%")
                    print("-" * 100)
