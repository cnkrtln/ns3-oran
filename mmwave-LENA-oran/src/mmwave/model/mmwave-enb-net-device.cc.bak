  ...
  if (!m_stopSendingMessages && m_hasValidSubscription)
  {
    ...
    if (!sub_map.empty())
    {
      try 
      {
        ...
        // DEBUG: Add PRB calculation debugging
        printf("=== NS3-DEBUG: PRB Calculation ===\n");
        printf("  - Current PRB Average: %f\n", currentPrbAvg);
        printf("  - Threshold: %d\n", threshold);
        printf("  - History size: %zu/%zu\n", m_prbHistory.size(), MAX_PRB_HISTORY);
        printf("  - Has enough points: %s\n", (currentPrbAvg >= 0) ? "YES" : "NO");
...
{
  // DEBUG: Always show debug output for CU-UP
  printf("=== CU-UP DEBUG (ALWAYS) ===\n");
  printf("  - Cell ID: %d\n", m_cellId);
  printf("  - UE Map size: %zu\n", m_rrc->GetUeMap().size());
  printf("  - m_sendCuUp: %s\n", m_sendCuUp ? "true" : "false");
  ...
  if (!indicationMessageHelper->IsOffline ())
    {
      // DEBUG: Add debug output for CU-UP
      printf("=== CU-UP DEBUG ===\n");
      printf("  - Cell ID: %d\n", m_cellId);
      printf("  - UE Map size: %zu\n", m_rrc->GetUeMap().size());
      printf("  - Cell DL TX Volume: %.2f\n", cellDlTxVolume);
      printf("  - Cell DL RX Volume: %.2f\n", cellDlRxVolume);
      printf("  - Per User Average Latency Sum: %.2f\n", perUserAverageLatencySum);
  ...
  if (m_forceE2FileLogging)
    ...
    {
      // DEBUG: Add debug output before creating indication message
...
MmWaveEnbNetDevice::BuildRicIndicationMessageCuCp (std::string plmId)
{
  ...
  if (!indicationMessageHelper->IsOffline ())
    {
      // DEBUG: Add debug output for CU-CP
      printf("=== CU-CP DEBUG ===\n");
      printf("  - Cell ID: %d\n", m_cellId);
      printf("  - UE Map size: %zu\n", ueMap.size());
      printf("  - L3 SINR map size: %zu\n", m_l3sinrMap.size());
...
{  
  // DEBUG: Always show debug output for DU
  printf("=== DU DEBUG (ALWAYS) ===\n");
  printf("  - Cell ID: %d\n", m_cellId);
  printf("  - UE Map size: %zu\n", m_rrc->GetUeMap().size());
  printf("  - m_sendDu: %s\n", m_sendDu ? "true" : "false");
  printf("  - m_forceE2FileLogging: %s\n", m_forceE2FileLogging ? "true" : "false");
...
{
  // DEBUG: Always show debug output at the start
  printf("\n=== MAIN DEBUG (ALWAYS) ===\n");
  printf("  - Cell ID: %d\n", m_cellId);
  ...
  if (m_sendCuUp)
    {
      ...
      // DEBUG: Add KPM indication debugging
      printf("=== NS3-DEBUG: KPM Indication Generation ===\n");
      printf("  - m_sendCuUp: %s\n", m_sendCuUp ? "true" : "false");
      if (header)
        printf("  - Header: valid (size: %zu)\n", header->m_size);
      else
        printf("  - Header: NULL\n");
      if (cuUpMsg)
        printf("  - CU-UP Message: valid (size: %zu)\n", cuUpMsg->m_size);
      else
        printf("  - CU-UP Message: NULL\n");

      // Send CU-UP only if offline logging is disabled
      if (header != nullptr && cuUpMsg != nullptr)
        {
          NS_LOG_DEBUG ("Send NR CU-UP");
          printf("  - Sending CU-UP KPM indication (size: %zu)\n", cuUpMsg->m_size);
        ...
        {
          printf("  - CU-UP KPM indication NOT sent (header or message is null)\n");
        }
    }
  ...
  if (m_sendCuCp)
    {
      ...
      // DEBUG: Add CU-CP debugging
      printf("=== NS3-DEBUG: CU-CP KPM Indication ===\n");
      printf("  - m_sendCuCp: %s\n", m_sendCuCp ? "true" : "false");
      if (header)
        printf("  - Header: valid (size: %zu)\n", header->m_size);
      else
      ...
      if (header != nullptr && cuCpMsg != nullptr)
        ...
        {
          printf("  - CU-CP KPM indication NOT sent (header or message is null)\n");
        }
    }
